...
static constexpr const uint16_t server_port = 42001;
...
int main (int argc, char* argv[])
{
    ...
    //
    iom::tls_adapter tls (sock);

    // Do a TLS handshake with the server
    //
    if (tls.start_client_tls(iom::tls_config(false), default_timeout)) {
        perror ("tls.start_client_tls");
        return 1;
    }
    ...
    while (true) {
        ...
        //
        if (tls.write(text.c_str(), text.size()) < 0) {
        ...
        //
        char buf[4096];
        auto result = tls.read (buf, sizeof(buf));
        if (result <= 0) {
            if (result == 0)
