 ...
 */
#include <iomultiplex/tls_adapter.hpp>
...
#define THIS_FILE "tls_adapter.cpp"
#ifdef TRACE_DEBUG
#  include <sys/types.h>
...
namespace iomultiplex {
    ...
    //--------------------------------------------------------------------------
    //--------------------------------------------------------------------------
    tls_adapter::tls_adapter (connection& conn, bool close_on_destruct)
    ...
    //--------------------------------------------------------------------------
    //--------------------------------------------------------------------------
    tls_adapter::tls_adapter (std::shared_ptr<connection> conn_ptr)
        : adapter (conn_ptr),
          tls_ctx (nullptr),
          tls (nullptr),
          mem_bio (nullptr),
    ...
    //--------------------------------------------------------------------------
    //--------------------------------------------------------------------------
    bool tls_adapter::is_tls_active () const
    {
    ...
    //--------------------------------------------------------------------------
    //--------------------------------------------------------------------------
    void tls_adapter::update_error (const char* msg)
    {
    ...
    //--------------------------------------------------------------------------
    //--------------------------------------------------------------------------
    unsigned long tls_adapter::last_error () const
    {
        return last_err;
    }
    ...
    //--------------------------------------------------------------------------
    //--------------------------------------------------------------------------
    std::string tls_adapter::last_error_msg () const
    {
        return last_err_msg;
    }
    ...
    //--------------------------------------------------------------------------
    //--------------------------------------------------------------------------
    x509_t tls_adapter::peer_cert () const
    {
        x509_t x509;
        if (tls_active)
    ...
    //--------------------------------------------------------------------------
    //--------------------------------------------------------------------------
    std::string tls_adapter::cipher_name () const
    {
    ...
    //--------------------------------------------------------------------------
    //--------------------------------------------------------------------------
    std::string tls_adapter::proto_ver () const
    {
    ...
    //--------------------------------------------------------------------------
    //--------------------------------------------------------------------------
    bool tls_adapter::verify_peer (long& error_code) const
    {
    ...
    //--------------------------------------------------------------------------
    //--------------------------------------------------------------------------
    bool tls_adapter::verify_peer (long& error_code, std::string& error_message) const
    {
    ...
    //--------------------------------------------------------------------------
    //--------------------------------------------------------------------------
    static int set_tls_cert_files (SSL_CTX* ctx, const tls_config& cfg)
    {
    ...
    //--------------------------------------------------------------------------
    //--------------------------------------------------------------------------
    static int set_tls_ciphers (SSL_CTX* ctx, const tls_config& cfg)
    {
    ...
    //--------------------------------------------------------------------------
    //--------------------------------------------------------------------------
    static SSL_CTX* configure_tls (const tls_config& tls_cfg, bool is_server, bool use_dtls)
    {
    ...
    //--------------------------------------------------------------------------
    //--------------------------------------------------------------------------
    int tls_adapter::start_tls (const tls_config& tls_config,
                                bool is_server,
                                bool use_dtls,
                                void* buf,
                                size_t buf_len,
                                ...
                                unsigned timeout)
    {
        ...
        start_result = start_tls (tls_config,
                                  ...
                                  buf,
                                  buf_len,
                                  [this, &tls_error, &errnum, &io_done](tls_adapter& ta, int e){
    ...
    //--------------------------------------------------------------------------
    //--------------------------------------------------------------------------
    int tls_adapter::initiate_tls_server_handshake (void* buf,
                                                    size_t buf_len,
    ...
    //--------------------------------------------------------------------------
    //--------------------------------------------------------------------------
    bool tls_adapter::handle_tls_handshake (bool is_server,
                                            tls_handshake_cb_t cb,
                                            unsigned timeout,
                                            int errnum)
    {
    ...
    //--------------------------------------------------------------------------
    //--------------------------------------------------------------------------
    void tls_adapter::clear_resources ()
    {
    ...
    //--------------------------------------------------------------------------
    //--------------------------------------------------------------------------
    int tls_adapter::shutdown (tls_handshake_cb_t callback, unsigned timeout)
    {
        // Sanity check
        if (handle() < 0) {
            errno = EBADF;
    ...
    // I/O handler context
    //--------------------------------------------------------------------------
    void tls_adapter::handle_tls_shutdown (tls_handshake_cb_t callback,
                                           unsigned timeout,
                                           int errnum)
    {
        if (errnum) {
    ...
    //--------------------------------------------------------------------------
    //--------------------------------------------------------------------------
    ssize_t tls_adapter::do_read (void* buf, size_t size, int& errnum)
    {
        if (!is_tls_active())
    ...
    //--------------------------------------------------------------------------
    //--------------------------------------------------------------------------
    ssize_t tls_adapter::do_write (const void* buf, size_t size, int& errnum)
    {
        if (!is_tls_active())
