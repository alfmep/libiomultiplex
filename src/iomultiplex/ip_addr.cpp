...
namespace iomultiplex {
    ...
    const ip_addr ipv4_addr_any (0, 0);
    const ip_addr ipv6_addr_any (0,0,0,0,0,0,0,0, 0);
    ...
    //--------------------------------------------------------------------------
    //--------------------------------------------------------------------------
    ip_addr::ip_addr (const ip_addr& addr)
    {
    ...
    //--------------------------------------------------------------------------
    //--------------------------------------------------------------------------
    ip_addr::ip_addr (const struct sockaddr_in& saddr)
    {
        memset (&sa, 0, sizeof(sa));
    ...
    //--------------------------------------------------------------------------
    //--------------------------------------------------------------------------
    ip_addr::ip_addr (const struct sockaddr_in6& saddr)
    {
        memset (&sa, 0, sizeof(sa));
    ...
    //--------------------------------------------------------------------------
    //--------------------------------------------------------------------------
    ip_addr::ip_addr (uint32_t ipv4_addr, uint16_t port_num)
    {
        memset (&sa, 0, sizeof(sa));
        ipv4 (ipv4_addr);
        port (port_num);
    ...
    //--------------------------------------------------------------------------
    //--------------------------------------------------------------------------
    ip_addr::ip_addr (uint8_t a, uint8_t b, uint8_t c, uint8_t d, uint16_t port_num)
    {
        memset (&sa, 0, sizeof(sa));
        uint32_t addr;
        addr  = ((uint32_t)a) << 24;
    ...
    //--------------------------------------------------------------------------
    //--------------------------------------------------------------------------
    ip_addr::ip_addr (const std::string& address)
    {
        memset (&sa, 0, sizeof(sa));
        if (!parse(address))
    ...
    //--------------------------------------------------------------------------
    //--------------------------------------------------------------------------
    ip_addr::ip_addr (const std::string& address, uint16_t port_num)
    {
        memset (&sa, 0, sizeof(sa));
        if (!parse(address))
            throw std::invalid_argument ("Invalid IPv[4|6] address.");
    ...
    //--------------------------------------------------------------------------
    //--------------------------------------------------------------------------
    ip_addr& ip_addr::operator= (const ip_addr& addr)
    {
        memcpy (&sa, &addr.sa, sizeof(sa));
        return *this;
    }
    ...
    //--------------------------------------------------------------------------
    //--------------------------------------------------------------------------
    ip_addr& ip_addr::operator= (const struct sockaddr_in& saddr)
    {
        memset (&sa, 0, sizeof(sa));
    ...
    //--------------------------------------------------------------------------
    //--------------------------------------------------------------------------
    ip_addr& ip_addr::operator= (const struct sockaddr_in6& saddr)
    {
        memset (&sa, 0, sizeof(sa));
        memcpy (&sa, &saddr, sizeof(struct sockaddr_in6));
        return *this;
    ...
    //--------------------------------------------------------------------------
    //--------------------------------------------------------------------------
    bool ip_addr::operator== (const ip_addr& addr) const
    {
    ...
    */
    ...
    //--------------------------------------------------------------------------
    //--------------------------------------------------------------------------
    bool ip_addr::operator< (const ip_addr& rhs) const
    {
    ...
    */
    ...
    //--------------------------------------------------------------------------
    //--------------------------------------------------------------------------
    bool ip_addr::parse (const std::string& address, const bool also_parse_port)
    {
        std::string ip_str;
        bool success = false;
        bool try_ipv4 = true;
    ...
    //--------------------------------------------------------------------------
    //--------------------------------------------------------------------------
    void ip_addr::port (const uint16_t port_num)
    {
        switch (((struct sockaddr&)sa).sa_family) {
        case AF_INET:
    ...
    //--------------------------------------------------------------------------
    //--------------------------------------------------------------------------
    void ip_addr::ipv4 (const uint32_t addr)
    {
    ...
    //--------------------------------------------------------------------------
    //--------------------------------------------------------------------------
    std::array<uint16_t, 8> ip_addr::ipv6 () const
    {
        std::array<uint16_t, 8> addr;
    ...
    //--------------------------------------------------------------------------
    //--------------------------------------------------------------------------
    void ip_addr::ipv6 (const std::array<uint16_t, 8>& addr)
    {
    ...
    //--------------------------------------------------------------------------
    //--------------------------------------------------------------------------
    std::shared_ptr<sock_addr> ip_addr::clone () const
    {
        return std::make_shared<ip_addr> (*this);
    }
    ...
    //--------------------------------------------------------------------------
    //--------------------------------------------------------------------------
    std::string ip_addr::to_string (bool include_port) const
    {
        std::stringstream ss;
